    ...
    U: EventListener + Send + 'static,
{
        ...
        X: Write,
    {
        ...
        loop {
            let mut finish_after_parse = false;

            // Read from the PTY.
            match self.pty.reader().read(&mut buf[unprocessed..]) {
                // This is received on Windows/macOS when no more data is readable from the PTY.
                Ok(0) if unprocessed == 0 => break,
                Ok(0) => finish_after_parse = true,
            ...
            let terminal = match &mut terminal {
                ...
                None => terminal.insert(match self.terminal.try_lock_unfair() {
                    // Force block if another read could discard buffered data.
                    None if finish_after_parse || unprocessed >= READ_BUFFER_SIZE => {
...
mod tests {
    ...
    use super::*;
    use crate::event::{VoidListener, WindowSize};
    ...
    impl Read for BytesThenError {
        fn read(&mut self, buffer: &mut [u8]) -> io::Result<usize> {
            match self.step {
                ...
                _ => {
                    let _ = self.error_reached.send(());
                    Err(io::Error::new(ErrorKind::BrokenPipe, "PTY closed"))
                },
            }
        }
    }
    ...
    struct ClosingPty {
        reader: BytesThenError,
        writer: io::Sink,
    }

    impl tty::EventedReadWrite for ClosingPty {
        type Reader = BytesThenError;
        type Writer = io::Sink;
    ...
    impl tty::EventedPty for ClosingPty {
        fn next_child_event(&mut self) -> Option<tty::ChildEvent> {
            None
        }
    }

    impl event::OnResize for ClosingPty {
        fn on_resize(&mut self, _: WindowSize) {}
    }

    #[test]
    fn parses_buffered_bytes_before_returning_a_terminal_read_error() {
        let (error_reached, wait_for_error) = mpsc::channel();
        let pty = ClosingPty {
            reader: BytesThenError { step: 0, error_reached },
            writer: io::sink(),
        };
        ...
        let mut event_loop = EventLoop::new(terminal.clone(), VoidListener, pty, true, false)
        ...
        std::thread::scope(|scope| {
            let drain = scope.spawn(move || {
                let mut state = State::default();
                let mut buffer = [0; 16];
                event_loop.pty_read::<io::Sink>(&mut state, &mut buffer, None)
            });

            wait_for_error.recv().expect("reader reached terminal error");
            drop(terminal_guard);
            let error = drain
                .join()
                .expect("drain thread panicked")
                .expect_err("terminal error was not returned after parsing");
            assert_eq!(error.kind(), ErrorKind::BrokenPipe);
        });

        assert_eq!(terminal.lock().grid()[Line(0)][Column(0)].c, 'X');
    }
}
