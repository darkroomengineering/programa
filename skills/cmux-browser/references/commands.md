...
```bash
cmux browser open <url>                        # opens in caller's workspace (uses PROGRAMA_WORKSPACE_ID)
...
> **Workspace context:** `browser open` targets the workspace of the terminal where the command is run (via `PROGRAMA_WORKSPACE_ID`), even if a different workspace is currently focused. Use `--workspace` to override.
