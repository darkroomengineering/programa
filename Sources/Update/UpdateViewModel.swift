...
class UpdateViewModel: ObservableObject {
    ...
    static func userFacingErrorMessage(for error: Swift.Error) -> String {
        ...
        if let networkError = networkError(from: nsError) {
            switch networkError.code {
            case NSURLErrorNotConnectedToInternet:
                return String(localized: "update.error.noInternet.message", defaultValue: "Programa can’t reach the update server. Check your internet connection and try again.")
            case NSURLErrorTimedOut:
            ...
            case NSURLErrorCannotConnectToHost:
                return String(localized: "update.error.serverUnreachable.message", defaultValue: "Programa couldn’t connect to the update server. Check your connection or try again later.")
            case NSURLErrorNetworkConnectionLost:
        ...
        if nsError.domain == SUSparkleErrorDomain {
            switch nsError.code {
            case 2001:
                return String(localized: "update.error.feedDownload.message", defaultValue: "Programa couldn't download the update feed. Check your connection and try again.")
            case 1000, 1002:
            ...
            case 1003, 1005, 4005:
                return String(localized: "update.error.permissionError.message", defaultValue: "Move Programa into Applications and relaunch to enable updates.")
            default:
                break
            }
        }
